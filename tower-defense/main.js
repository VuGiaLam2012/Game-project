...
// CÁC ELEMENT BÊN PHẢI CỦA KHU VỰC INVENTORY MỚI
const selectedTowerImg = document.getElementById('selected-tower-img');
const selectedTowerPlaceholder = document.getElementById('selected-tower-placeholder');
const selectedTowerName = document.getElementById('selected-tower-name');
const selectedTowerStatsContent = document.getElementById('selected-tower-stats-content');
...
// =========================================================
// HÀM HIỂN THỊ CHI TIẾT TOWER Ở CỘT BÊN PHẢI INVENTORY
// =========================================================
function updateInventoryPreview(type) {
  if (!type) {
    if (selectedTowerImg) selectedTowerImg.classList.add('hidden');
    if (selectedTowerPlaceholder) selectedTowerPlaceholder.classList.remove('hidden');
    if (selectedTowerName) selectedTowerName.textContent = 'Tên tower';
    if (selectedTowerStatsContent) selectedTowerStatsContent.innerHTML = '<p>Chưa chọn tower</p>';
    return;
  }

  currentPreviewTypeId = type.id;

  // 1. Tự động Render hình ảnh 2D của Tower lên Preview Box
  if (selectedTowerImg && selectedTowerPlaceholder) {
    try {
      ...
      const skin = game.towerSkins[type.id] || 'classic';
      const tempTower = new type.TowerClass(0, 0, pCtx, 100, () => {}, skin);
      tempTower.x = 50;
      tempTower.y = 50;
      tempTower.draw();
      selectedTowerImg.src = previewCanvas.toDataURL();
      selectedTowerImg.classList.remove('hidden');
      selectedTowerPlaceholder.classList.add('hidden');
    } catch (e) {
      selectedTowerImg.classList.add('hidden');
      selectedTowerPlaceholder.classList.remove('hidden');
    }
  }
  ...
  // 3. Stats của Tower
  if (selectedTowerStatsContent) {
    const dummy = new type.TowerClass(0, 0, ctx, GRID_SIZE, () => {});
    const unlocked = isTowerUnlocked(type);
    const equipped = isTowerEquipped(type);

    let statsHtml = `
      <div style="display:flex; flex-direction:column; gap:6px;">
        <div><strong>Placement Cost:</strong> $${type.cost}</div>
        <div><strong>Unlock Level:</strong> Level ${type.unlockLevel}</div>
    `;

    if (dummy.incomePerTick) {
      statsHtml += `<div><strong>Income:</strong> $${dummy.incomePerTick} / ${dummy.incomeInterval}s</div>`;
    } else {
      statsHtml += `
        <div><strong>Damage:</strong> ${dummy.dmg}</div>
        <div><strong>Range:</strong> ${(dummy.range / GRID_SIZE).toFixed(1)} tiles</div>
        <div><strong>Fire Rate:</strong> ${dummy.fireRate ? dummy.fireRate.toFixed(2) + '/s' : 'N/A'}</div>
      `;
    }

    statsHtml += `
        <div><strong>Max Level:</strong> ${dummy.maxLevel || 3}</div>
        <div><strong>Skin:</strong> ${game.towerSkins[type.id] || 'Classic'}</div>
        <div style="margin-top:10px;">
    `;

    if (!unlocked) {
      const canUnl = canUnlockTower(type);
      const req = type.unlockGems ? `${type.unlockGems} gems` : `$${type.unlockCost}`;
      statsHtml += `<button id="preview-action-btn" class="secondary-btn" style="width:100%" ${!canUnl ? 'disabled' : ''}>Unlock (${req})</button>`;
    } else {
      statsHtml += `<button id="preview-action-btn" class="primary-btn" style="width:100%">${equipped ? 'Unequip Unit' : 'Equip Unit'}</button>`;
    }

    statsHtml += `</div></div>`;
    selectedTowerStatsContent.innerHTML = statsHtml;
...
function renderLoadout() {
  ...
  towerTypes.forEach((type) => {
    ...
    btn.innerHTML = `<span class="loadout-icon" style="background:${unlocked ? type.color : '#475569'}"></span><span><strong>${type.name}</strong><small>${unlocked ? (equipped ? 'Equipped' : 'Owned') : unlockLabel}</small></span><span class="loadout-state">${equipped ? 'ON' : unlocked ? 'OWNED' : 'LOCKED'}</span>`;
    btn.disabled = !unlocked && !unlockReady;
    
    // BẤM VÀO TOWER BÊN TRÁI SẼ CẬP NHẬT XEM BÊN PHẢI & XỬ LÝ TRANG BỊ
    btn.addEventListener('click', () => {
      updateInventoryPreview(type);
      if (!unlocked) {
        unlockTower(type);
  ...
  // Mặc định preview tower đầu tiên nếu chưa chọn
  if (!currentPreviewTypeId && towerTypes.length > 0) {
    updateInventoryPreview(towerTypes[0]);
  } else if (currentPreviewTypeId) {
    const curType = towerTypes.find(t => t.id === currentPreviewTypeId);
    if (curType) updateInventoryPreview(curType);
  }
