...
const gameModes = [
  { id: 'easy', name: 'Easy', description: '20 waves with forgiving enemy health and speed.', lives: 20, hpScale: 0.8, speedScale: 0.85, rewardScale: 0.8, maxWaves: 20 },
  { id: 'molten', name: 'Molten', description: '40 waves of faster enemies and rising pressure.', lives: 15, hpScale: 1, speedScale: 1, rewardScale: 1, maxWaves: 40 },
  { id: 'fallen', name: 'Fallen', description: '40 waves with stronger and faster enemies.', lives: 10, hpScale: 1.35, speedScale: 1.15, rewardScale: 1.35, maxWaves: 40, gemRewardScale: 1.5 },
  { id: 'hardcore', name: 'Hardcore', description: '50 waves, one life, extreme enemies, and gem rewards.', lives: 1, hpScale: 1.8, speedScale: 1.35, rewardScale: 1.5, gemRewardScale: 3, maxWaves: 50, finalGemReward: 500 }
];
...
function buildWaveQueue(wave, mode) {
  ...
  const hardcore = mode.id === 'hardcore';
  ...
  for (let i = 0; i < total; i++) {
    ...
    if (bossWave && i === total - 1) {
      type = 'boss';
    } else if (hardcore && wave >= 10 && roll < 0.22) {
      type = 'brute';
    } else if (hardcore && wave >= 5 && roll < 0.5) {
      type = 'tank';
    } else if (hardcore && wave >= 2 && roll < 0.75) {
      type = 'runner';
    } else if (wave >= 8 && roll < 0.12) {
      type = 'brute';
    } else if (wave >= 5 && roll < 0.26) {
      type = 'tank';
    } else if (wave >= 3 && roll < 0.48) {
      type = 'runner';
    }
