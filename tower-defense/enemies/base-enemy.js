export class BaseEnemy {
  ...
  damage(dmg) {
    const damageValue = Math.max(1, Math.round(dmg));
    this.hp -= dmg;
    this.hitFlash = 0.22;
    this.damagePopups.push({
      ...
      y: this.y - 12,
      life: 0.7,
      color: this.type === 'boss' ? '#fecaca' : '#f8fafc'
    });
    ...
    if (this.hp <= 0) {
      ...
      if (this.game) {
        const rewardScale = this.game.rewardScale ?? 1;
        this.game.money += Math.round(this.reward * rewardScale);
        const baseXpReward = this.type === 'boss' ? 80 : Math.max(8, Math.round(this.reward * 1.5));
        this.game.addXp(Math.round(baseXpReward * rewardScale));
        this.game.syncHud();
      }
    }
  }
  ...
  drawDamagePopup() {
    for (const popup of this.damagePopups) {
      const alpha = Math.max(0, popup.life / 0.7);
      this.ctx.save();
